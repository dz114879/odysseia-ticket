...
class BasicSettingsModal(discord.ui.Modal, title="基础设置"):
    def __init__(self, *, guild_id: int, config: GuildConfigRecord) -> None:
        ...
        self.timezone_input = discord.ui.TextInput(label="时区", placeholder="例如 Asia/Shanghai", default=config.timezone, max_length=50, required=False, row=0)
        self.max_tickets_input = discord.ui.TextInput(label="活跃工单上限", placeholder="1-1000", default=str(config.max_open_tickets), max_length=5, required=False, row=1)
